  ...
  <Table :api-client-function="apiClientFunction">
    <template #table-headers>
      <tr>
        ...
        <th scope="col" class="py-2 pr-2 font-semibold">Dividend</th>
        <th scope="col" class="py-2 pr-2 font-semibold">Quantity Per Unit</th>
    ...
    <template #table-rows="{ data }">
      <tr v-for="(dividend, index) in data" :key="index">
        <td v-if="!props.address" class="whitespace-nowrap py-3 pr-3 min-w-[100px]">
          <NuxtLink :to="`/address/${dividend.source}`" class="font-medium leading-6 text-base text-white">
            {{ dividend.source }}
          </NuxtLink>
        </td>
        <td class="whitespace-nowrap py-3 pr-3 min-w-64">
          <div class="flex items-center gap-x-4">
            <NuxtImg :src="`https://api.xcp.io/img/icon/${formatAssetName(dividend.asset, dividend.asset_info)}`" :alt="formatAssetName(dividend.asset, dividend.asset_info)" class="h-10 w-10 bg-gray-800" loading="lazy" />
            <NuxtLink :to="`/asset/${formatAssetName(dividend.asset, dividend.asset_info)}`" class="font-medium leading-6 text-base text-white">
        ...
        <td class="whitespace-nowrap py-3 pr-3 min-w-64">
          <div class="flex items-center gap-x-4">
            <NuxtImg :src="`https://api.xcp.io/img/icon/${formatAssetName(dividend.dividend_asset, dividend.dividend_asset_info)}`" :alt="formatAssetName(dividend.dividend_asset, dividend.dividend_asset_info)" class="h-10 w-10 bg-gray-800" loading="lazy" />
            <NuxtLink :to="`/asset/${formatAssetName(dividend.dividend_asset, dividend.dividend_asset_info)}`" class="font-medium leading-6 text-base text-white">
        ...
        <td class="whitespace-nowrap py-3 pl-0 pr-8 text-base leading-6 text-gray-300 md:table-cell">
          <NuxtLink :to="`/block/${dividend.block_index}`" class="leading-6 text-white">
        ...
        <td class="whitespace-nowrap py-3 pl-0 text-base font-medium text-right">
          <NuxtLink :to="`/tx/${dividend.tx_hash}`" class="text-primary">View</NuxtLink>
        </td>
      </tr>
    </template>
  </Table>
