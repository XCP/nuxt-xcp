  ...
  <TableTemplate :api-client-function="apiClientFunction">
    <template #table-headers>
      <tr>
        ...
        <th scope="col" class="py-2 pr-2 font-semibold min-w-[100px]">Asset</th>
        <th scope="col" class="py-2 pr-2 font-semibold min-w-[200px] hidden xl:table-cell">Description</th>
      ...
      <tr v-for="(asset, index) in data" :key="index">
        <td class="whitespace-nowrap py-3 pr-2">
          <div class="flex items-center gap-x-4 leading-6">
            <span v-if="asset.locked" class="inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset bg-green-400/10 text-green-400 ring-green-400/20">
        ...
        <td class="whitespace-nowrap py-3 pr-3 min-w-[100px]">
          <div class="flex items-center justify-between gap-x-4">
            <div class="flex items-center gap-x-4">
              <NuxtImg :src="`https://api.xcp.io/img/icon/${asset.asset}`" :alt="asset.asset" class="h-10 w-10 bg-gray-800" loading="lazy" />
              <NuxtLink :to="`/asset/${formatAssetName(asset.asset, asset)}`" class="font-medium leading-6 text-base text-white">
        ...
        <td class="whitespace-nowrap py-3 pr-3 text-base leading-6 text-gray-300 hidden xl:table-cell">
          <div class="truncate" :title="asset.description">{{ formatDescription(asset.description, 100) }}</div>
        </td>
        ...
        <td class="whitespace-nowrap py-3 pl-0 text-base font-medium text-right">
          <NuxtLink :to="`/asset/${formatAssetName(asset.asset, asset)}`" class="text-primary">View</NuxtLink>
        </td>
      </tr>
    </template>
  </TableTemplate>
