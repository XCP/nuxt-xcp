  ...
  <div>
    ...
    >
      ...
      <NuxtImg
        ...
        loading="lazy"
        :class="[props.apiData ? 'rounded-3xl' : 'rounded-2xl']"
      />
      <div
        class="absolute inset-0 bg-black bg-opacity-100 flex justify-center items-center text-white text-xl font-semibold rounded-xl cursor-pointer transition-opacity duration-2000ms ease-in-out"
    ...
    <NuxtImg
      ...
      :alt="computedAltText"
      class="w-full h-full object-contain"
      :class="{ 'image-rendering-pixelated': shouldPixelate, 'rounded-3xl': props.apiData, 'rounded-2xl': !props.apiData }"
      loading="lazy"
    />
  </div>
</template>
