<template>
  ...
  <div class="sm:hidden">
    ...
    <select id="tabs" class="block w-full text-white bg-gray-800 rounded text-base focus:outline-none md:w-auto" @change="emitTabChange($event.target.value)">
      <option v-for="tab in tabs" :key="tab.hash" :value="tab.name" :selected="tab.name === activeTab">{{ tab.name }}</option>
    </select>
  </div>
  ...
  <div class="hidden sm:block mb-6">
    <nav class="flex" aria-label="Tabs">
      <ul class="flex w-full border-b border-white/5">
        <li v-for="tab in visibleTabs" :key="tab.hash" class="last:mr-0">
          <button
            :class="[
        ...
        <li v-if="hiddenTabs.length > 0" class="relative">
          <Menu as="div" class="relative">
            ...
            >
              <MenuItems class="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                <MenuItem v-for="tab in hiddenTabs" :key="tab.hash">
                  <button class="flex items-center px-4 py-2 text-base text-gray-700 hover:bg-gray-100 w-full" @click="emitTabChange(tab.name)">
                    {{ tab.name }}
                  </button>
                </MenuItem>
              </MenuItems>
