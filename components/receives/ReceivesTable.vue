      ...
      <tr v-for="(receive, index) in data" :key="index">
        <td class="whitespace-nowrap py-3 pr-3 min-w-64">
          <div class="flex items-center gap-x-4">
            <NuxtImg :src="`https://api.xcp.io/img/icon/${formatAssetName(receive.asset, receive.asset_info)}`" :alt="formatAssetName(receive.asset, receive.asset_info)" class="h-10 w-10 bg-gray-800" loading="lazy" />
            <NuxtLink :to="`/asset/${formatAssetName(receive.asset, receive.asset_info)}`" class="font-medium leading-6 text-base text-white">
        ...
        <td class="whitespace-nowrap py-3 pr-3 text-base leading-6 text-gray-300">
          <Status :status="receive.status" />
        </td>
        <td class="whitespace-nowrap py-3 pl-0 pr-8 text-base leading-6 text-gray-300 md:table-cell">
          <NuxtLink :to="`/block/${receive.block_index}`" class="leading-6 text-white">
