<template>
  ...
  <!-- Grid View -->
  <div v-if="viewMode === 'grid' && state.assets.length > 0" class="grid grid-cols-2 sm:grid-cols-4 2xl:grid-cols-6 gap-4 mt-6">
    <div v-for="asset in state.assets" :key="asset.asset_name" class="flex flex-col overflow-hidden rounded-2xl border border-gray-700 rounded bg-gray-800">
  ...
  <div v-if="state.allDataLoaded && state.assets.length > 0 || state.assets.length > 0 && state.assets.length < 100" class="flex justify-center mt-16 sm:mt-20 lg:mt-24">
...
const fetchData = async () => {
  ...
  try {
    const response = await fetch(`https://api.xcp.io/api/collections?${query}`);
