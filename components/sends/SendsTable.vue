      ...
      >
        <td class="whitespace-nowrap py-3 pr-3 min-w-64">
          <div class="flex items-center gap-x-4">
            <NuxtImg
              :src="`https://app.xcp.io/img/icon/${formatAssetName(send.asset, send.asset_info)}`"
...
const apiClientFunction = (params = {}) => {
  ...
  if (props.address) {
    ...
    return $apiClient.getAssetSends(props.assetName, params)
  }
  else if (props.blockIndex) {
    return $apiClient.getBlockSends(props.blockIndex, params)
  }
  else {
    return $apiClient.getSends(params)
  }
}
</script>
