      ...
      <tr v-for="(send, index) in data" :key="index">
        <td class="whitespace-nowrap py-3 pr-3 min-w-64">
          <div class="flex items-center gap-x-4">
            <NuxtImg :src="`https://api.xcp.io/img/icon/${formatAssetName(send.asset, send.asset_info)}`" :alt="formatAssetName(send.asset, send.asset_info)" class="h-10 w-10 bg-gray-800" loading="lazy" />
            <NuxtLink :to="`/asset/${formatAssetName(send.asset, send.asset_info)}`" class="font-medium leading-6 text-base text-white">
        ...
        <td class="whitespace-nowrap py-3 pr-3 min-w-[100px]">
          <NuxtLink :to="`/address/${send.destination}`" class="font-medium leading-6 text-base text-white">
            {{ send.destination }}
          </NuxtLink>
        </td>
        <td class="whitespace-nowrap py-3 pr-3 text-base leading-6 text-gray-300">
