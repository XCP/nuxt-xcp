<template>
  <div>
    ...
    >
      <div class="text-center">
        ...
        <p class="text-base text-gray-400">
          The entire supply may be escrowed in a dex order or dispenser.
        </p>
      </div>
    </div>
    ...
    <!-- Grid View -->
    <div
      v-if="viewMode === 'grid' && state.balances.length > 0"
      class="grid grid-cols-2 sm:grid-cols-4 2xl:grid-cols-6 gap-4 mt-6"
    >
      <div
        v-for="balance in state.balances"
        :key="balance.asset"
      ...
      >
        ...
        <NuxtLink :to="`/asset/${balance.asset}`">
          ...
          <div class="aspect-w-5 aspect-h-7 w-full">
            <NuxtImg
              :src="`https://api.xcp.io/img/full/${balance.asset_name}`"
              :alt="balance.asset"
              class="rounded-2xl object-contain w-full h-full"
              :style="balance.asset.startsWith('A') ? 'image-rendering: pixelated' : null"
              loading="lazy"
            />
          </div>
    ...
    <!-- Table View -->
    <div
      v-else-if="state.balances.length > 0"
      class="mt-6 relative overflow-x-auto"
    >
      <TableTemplate class="table-auto w-full whitespace-nowrap text-left border-white/10">
          ...
          >
            <td class="whitespace-nowrap py-3 pr-3 min-w-64">
              <div class="flex items-center gap-x-4">
                <NuxtImg
                  :src="`https://api.xcp.io/img/icon/${balance.asset_name}`"
                  :alt="balance.asset"
