<template>
  ...
  <nav class="mt-6 sm:mt-0 flex items-center justify-between" aria-label="Pagination">
    <div class="flex items-center">
      <p v-if="state.transactions.length < totalTransactions" class="text-base text-gray-300 leading-9">
        Showing
  ...
  <!-- No Data -->
  <div v-if="state.transactions.length === 0 && !state.loading" class="my-10 flex justify-center items-center">
    ...
    <table class="w-full whitespace-nowrap text-left border-white/10">
      <thead class="border-t border-b border-white/10 text-base leading-6 text-white">
        <tr>
          ...
          <th scope="col" class="hidden xl:block py-2 pr-2 font-semibold">TX Hash</th>
          <th scope="col" class="py-2 w-20"><span class="sr-only">View</span></th>
        </tr>
      </thead>
      <tbody class="divide-y divide-white/5">
        <tr v-for="transaction in state.transactions" :key="transaction.tx_hash">
          ...
          <td class="whitespace-nowrap py-3 pr-3">
            <div class="flex items-center gap-x-4">
              <NuxtImg :src="`https://api.xcp.io/img/icon/${transaction.bindings.asset || transaction.bindings.give_asset}`" :alt="transaction.bindings.asset || transaction.bindings.give_asset" class="h-10 w-10 bg-gray-800" loading="lazy" />
            </div>
          </td>
          <td class="whitespace-nowrap py-3 pl-0 pr-3 text-base leading-6 text-gray-300">
