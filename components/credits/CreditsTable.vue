<template>
  <Table :api-client-function="apiClientFunction">
    ...
    <template #table-rows="{ data }">
      <tr v-for="(credit, index) in data" :key="index">
        <td class="whitespace-nowrap py-3 pr-3 min-w-64">
          <div class="flex items-center gap-x-4">
            <NuxtImg :src="`https://api.xcp.io/img/icon/${formatAssetName(credit.asset, credit.asset_info)}`" :alt="formatAssetName(credit.asset, credit.asset_info)" class="h-10 w-10 bg-gray-800" loading="lazy" />
            <NuxtLink :to="`/asset/${formatAssetName(credit.asset, credit.asset_info)}`" class="font-medium leading-6 text-base text-white">
        ...
        <td v-if="!props.address" class="whitespace-nowrap py-3 pr-3 min-w-[100px]">
          <NuxtLink :to="`/address/${credit.address}`" class="font-medium leading-6 text-base text-white">
            {{ credit.address }}
          </NuxtLink>
        </td>
        ...
        <td v-if="!props.blockIndex" class="whitespace-nowrap py-3 pl-0 pr-8 text-base leading-6 text-gray-300 md:table-cell">
          <NuxtLink :to="`/block/${credit.block_index}`" class="leading-6 text-white">
        ...
        <td class="whitespace-nowrap py-3 pl-3 text-right text-base font-medium">
          <NuxtLink :to="`/tx/${credit.event}`" class="text-primary" aria-label="View transaction details">View</NuxtLink>
        </td>
      </tr>
    </template>
  </Table>
