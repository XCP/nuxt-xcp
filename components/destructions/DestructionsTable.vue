<template>
  ...
  >
    ...
    <template #table-rows="{ data }">
      <tr
        v-for="(destruction, index) in data"
        :key="index"
      >
        <td class="whitespace-nowrap py-3 pr-3 min-w-64">
          <div class="flex items-center gap-x-4">
            <NuxtImg
              :src="`https://api.xcp.io/img/icon/${formatAssetName(destruction.asset, destruction.asset_info)}`"
              :alt="formatAssetName(destruction.asset, destruction.asset_info)"
              class="h-10 w-10"
              loading="lazy"
            />
            <NuxtLink
